...
export interface HorizontalRulerProps {
  ...
  onTabStopAdd?: (positionTwips: number) => void;
  onTabStopMove?: (oldPositionTwips: number, newPositionTwips: number) => void;
}

type MarkerType = 'leftMargin' | 'rightMargin' | 'topMargin' | 'bottomMargin' | 'firstLineIndent' | 'leftIndent' | 'rightIndent';
...
export function HorizontalRuler({
  ...
  const handleTabStopDragStart = useCallback(
    (e: React.MouseEvent, positionTwips: number) => {
      if (!editable) return;
      e.preventDefault();
      e.stopPropagation();
      setDraggingTabStop(positionTwips);
    },
    [editable]
  );
  ...
  const handleDrag = useCallback(
    (e: MouseEvent) => {
      ...
      if (!dragging && draggingTabStop === null) return;
      ...
      if (draggingTabStop !== null) {
        const rounded = Math.round(Math.max(0, Math.min(positionTwips, pageWidthTwips)));
      ...
      } else if (dragging === 'topMargin') {
        const maxMargin = pageWidthTwips - 720;
        const rounded = Math.round(Math.max(0, Math.min(positionTwips, maxMargin)));
        setDragValue(rounded);
        onTopMarginChange?.(rounded);
      } else if (dragging === 'bottomMargin') {
        const maxMargin = pageWidthTwips - 720;
        const fromRight = pageWidthTwips - positionTwips;
        const rounded = Math.round(Math.max(0, Math.min(fromRight, maxMargin)));
        setDragValue(rounded);
        onBottomMarginChange?.(rounded);
  ...
  const handleDragEnd = useCallback(() => {
    if (draggingTabStop !== null && dragValue !== null) {
      if (dragValue !== draggingTabStop) {
        onTabStopMove?.(draggingTabStop, dragValue);
      }
    }

    setDragging(null);
    setDraggingTabStop(null);
    setDragValue(null);
    setDragPositionPx(null);
  }, [draggingTabStop, dragValue, onTabStopMove]);

  useEffect(() => {
    if (dragging || draggingTabStop !== null) {
  ...
  const handleRulerClick = useCallback(
    (e: React.MouseEvent) => {
      if (!editable || !onTabStopAdd || !rulerRef.current) return;
      // Do not add a tab stop if clicking an existing marker or handle
      if ((e.target as HTMLElement).closest('.docx-ruler-indent, .docx-tab-stop')) return;

      const rect = rulerRef.current.getBoundingClientRect();
      const x = e.clientX - rect.left;
      const positionTwips = pixelsToTwips(x / zoom);

      // Snap to nearest 1/8th of the unit
      const unitTwips = unit === 'inch' ? TWIPS_PER_INCH : TWIPS_PER_CM;
      const snapTwips = Math.round(positionTwips / (unitTwips / 8)) * (unitTwips / 8);
      onTabStopAdd(snapTwips);
    },
    [editable, onTabStopAdd, zoom, unit]
  );
  ...
  return (
    ...
    >
      ...
      {/* Top margin drag zone (small bar at top edge) */}
      {editable && (onTopMarginChange) && (
        <div
          style={{
            position: 'absolute',
            top: 0,
            left: formatPx(leftMarginPx),
            right: formatPx(rightMarginPx),
            height: 6,
            backgroundColor: 'transparent',
            cursor: 'ns-resize',
            zIndex: 2,
      ...
      {/* Bottom margin drag zone (small bar at bottom edge) */}
      {editable && (onBottomMarginChange) && (
        <div
          style={{
            position: 'absolute',
            bottom: 0,
      ...
      {tabStops?.map((tab) => {
        const isDraggingThis = draggingTabStop === tab.position;
        const displayPx =
          isDraggingThis && dragPositionPx !== null
            ? dragPositionPx
            : twipsToPixels(tab.position) * zoom;

        return (
          <TabStopMarker
            key={tab.position}
            tabStop={tab}
            positionPx={displayPx}
            onDoubleClick={() => onTabStopRemove?.(tab.position)}
            onMouseDown={(e) => handleTabStopDragStart(e, tab.position)}
          />
        );
      })}

      {/* Drag tooltip */}
      {(dragging || draggingTabStop !== null) && dragValue !== null && dragPositionPx !== null && (
