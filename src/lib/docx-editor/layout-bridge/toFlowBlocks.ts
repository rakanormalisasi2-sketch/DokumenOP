...
function convertImage(node: PMNode, startPos: number, pageContentHeight?: number): ImageBlock {
  ...
  // Anchor images with 'behind', 'inFront', AND floating wrap types
  // Floating images (square, tight, through) get pulled out of paragraph flow
  // and rendered in a separate layer — mark them anchored so layoutEngine
  // routes them through layoutAnchoredImage instead of inline layoutImage
  const shouldAnchor =
    wrapType === 'behind' ||
    wrapType === 'inFront' ||
    (wrapType != null && ['square', 'tight', 'through'].includes(wrapType));
