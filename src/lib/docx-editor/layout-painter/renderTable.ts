...
function renderTableCell(
  ...
  // Apply borders - use cell borders if available, otherwise apply default Word borders
  // so table is visible even without explicit cell borders (A1 stability fix)
  if (cell.borders) {
    ...
    // No explicit borders — apply default single black border so table is visible
    const defaultBorder = { width: 4, color: '#000000', style: 'solid' as const };
    if (borderFlags.isFirstRow) applyBorder(cellEl, 'top', defaultBorder);
    applyBorder(cellEl, 'right', defaultBorder);
    applyBorder(cellEl, 'bottom', defaultBorder);
    if (borderFlags.isFirstCol) applyBorder(cellEl, 'left', defaultBorder);
  }
