    ...
    useImperativeHandle(
      ...
      () => ({
        ...
        setSelection(anchor: number, head?: number) {
          ...
          // Detect if anchor and head are inside different table cells
          try {
            let anchorCellPos = null;
            for (let d = $anchor.depth; d > 0; d--) {
              const name = $anchor.node(d).type.name;
              if (name === 'table_cell' || name === 'table_header') {
                anchorCellPos = $anchor.before(d);
                break;
              }
            }

            let headCellPos = null;
            for (let d = $head.depth; d > 0; d--) {
              const name = $head.node(d).type.name;
              if (name === 'table_cell' || name === 'table_header') {
                headCellPos = $head.before(d);
                break;
              }
            }

            if (anchorCellPos !== null && headCellPos !== null && anchorCellPos !== headCellPos) {
              const cellSel = CellSelection.create(state.doc, anchorCellPos, headCellPos);
              dispatch(state.tr.setSelection(cellSel));
              return;
            }
          } catch (e) {
            // Fallback to text selection gracefully if CellSelection fails
          }
