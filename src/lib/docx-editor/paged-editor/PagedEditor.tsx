...
import { TableHoverOverlay } from './TableHoverOverlay';
...
import { cleanupImageObserver } from '../layout-painter/renderImage';
  ...
  function PagedEditor(props, ref) {
    ...
    const buildImageSelectionInfo = useCallback(
      (el: HTMLElement, pmPos: number): ImageSelectionInfo => {
        ...
        if (view) {
          ...
          if (node && node.type.name === 'image' && node.attrs.transform) {
            const rotateMatch = /rotate\(([-0-9.]+)deg\)/.exec(node.attrs.transform);
            if (rotateMatch) {
    ...
    const updateSelectionOverlay = useCallback(
      (state: EditorState) => {
        ...
        // Determine if this is a CellSelection
        ...
        if (from === to) {
          ...
          // Range selection across multiple cells - handled by CSS .layout-table-cell-selected
          setSelectionRects([]);
          setCaretPosition(null);
        } else {
    ...
    /**
     * Handle image rotation from the overlay.
     */
    const handleImageRotate = useCallback((pmPos: number, newRotationDeg: number) => {
      const view = hiddenPMRef.current?.getView();
      if (!view) return;
      ...
      try {
        ...
        // Preserve other transform rules but replace/add rotation
        let transformStr = node.attrs.transform || '';
        transformStr = transformStr.replace(/rotate\([-0-9.]+deg\)\s*/g, '');
        transformStr = `${transformStr} rotate(${newRotationDeg}deg)`.trim();
    ...
    // Clean up IntersectionObserver for images when the editor unmounts.
    // This prevents memory leaks during SPA navigation where observers would
    // otherwise accumulate across document changes.
    useEffect(() => {
      return () => {
        const doc = options.document ?? document;
        cleanupImageObserver(doc);
      };
    }, [options.document]);
    ...
    return (
      ...
      >
        ...
        >
          ...
          {/* Table Hover Handle Overlay */}
          {!readOnly && (
            <TableHoverOverlay
              containerRef={pagesContainerRef}
              zoom={zoom}
              getEditorState={() => hiddenPMRef.current?.getState() || null}
              dispatchTransaction={(tr) => hiddenPMRef.current?.dispatch(tr)}
            />
          )}
