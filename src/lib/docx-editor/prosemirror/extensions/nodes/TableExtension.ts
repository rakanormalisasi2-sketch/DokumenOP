  ...
  onSchemaReady(ctx: ExtensionContext): ExtensionRuntime {
    ...
    function addRowAbove(state: EditorState, dispatch?: (tr: Transaction) => void): boolean {
      ...
      if (dispatch) {
        ...
        // A3: set selection to first cell content of the newly inserted row
        const firstCellPos = rowPos + 1; // after tableRow open tag
        const $cellPos = tr.doc.resolve(firstCellPos + 1); // inside first cell (cell tag + paragraph tag)
        tr.setSelection(TextSelection.create(tr.doc, $cellPos.pos));
        dispatch(tr.scrollIntoView());
      }
      return true;
    }
