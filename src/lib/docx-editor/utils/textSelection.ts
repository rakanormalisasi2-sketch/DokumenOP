...
/**
 * Regular expression for end-of-sentence punctuation
 */
 ...
 */
export function findWordBoundaries(text: string, position: number, includeTrailingSpace: boolean = true): [number, number] {
  ...
  if (isWordCharacter(charAtPosition)) {
    ...
    // Apply strict Word-like trailing space selection rules
    if (includeTrailingSpace) {
      let tempEnd = end;
      // Expand through trailing spaces
      while (tempEnd < text.length && isWhitespace(text[tempEnd])) {
        tempEnd++;
      }

      // Determine what follows the trailing spaces
      const nextChar = tempEnd < text.length ? text[tempEnd] : null;
      const isEndOfParagraph = nextChar === null;
      const isBeforePunctuation = nextChar !== null && END_PUNCTUATION_REGEX.test(nextChar);

      // If NOT at the end of paragraph and NOT before punctuation, include the spaces
      // In Word, trailing spaces are excluded if they immediately precede punctuation or the end of a paragraph.
      if (!isEndOfParagraph && !isBeforePunctuation && tempEnd > end) {
        end = tempEnd;
      }
    }
